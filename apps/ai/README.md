...
> 별도 경로 `POST /v1/llm/analysis-criteria/resolve` 는 dev 의 로컬 키워드 매칭 구현(maetelson) 이 점유합니다.
...
```
POST /v1/llm/analysis-results/describe-llm
```
...
> 별도 경로 `POST /v1/llm/analysis-results/describe` 는 dev 의 템플릿 기반 구현(maetelson) 이 점유합니다.
