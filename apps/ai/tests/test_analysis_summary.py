...
def test_describe_llm_endpoint_not_registered() -> None:
    """임시 충돌 회피 경로였던 describe-llm은 등록하지 않습니다."""
...
def test_describe_endpoint_registered_once() -> None:
    """결과 요약 POST /describe 라우터가 한 번만 등록되는지 검증합니다."""

    routes = [
        route
        for route in app.routes
        if getattr(route, "path", None) == "/v1/llm/analysis-results/describe"
