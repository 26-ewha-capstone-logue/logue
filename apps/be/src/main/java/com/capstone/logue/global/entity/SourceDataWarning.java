...
/**
 * 데이터 소스({@link DataSource}) 수준에서 감지된 데이터 품질 경고를 저장하는 엔티티입니다.
 *
 * <p>파일 업로드 및 스키마 분석 과정에서 발견된 이슈를 사용자에게 알리기 위해 생성됩니다.
 * 내부 식별 코드({@link SourceWarningKey})와 사용자 노출 문구({@code comment})를 모두 보유하여,
...
@Getter
@Entity
@Table(name = "source_data_warnings")
@Builder
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor(access = AccessLevel.PRIVATE)
public class SourceDataWarning extends BaseTimeEntity {
