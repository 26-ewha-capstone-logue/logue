...
`self_demo`는 FE가 시연 화면을 기준으로 질문 해석 결과를 직접 재현하는 자체 시연 플로우다. AI 실행과 FE 화면을 분리한 현재 구조를 유지하면서, 저장된 결과와 실시간 실행 결과를 모두 설명할 수 있게 준비한다.

#### 0. 시연 목적

| 목적             | 설명                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------ |
| 핵심 가치 전달   | 자연어 질문이 분석 기준(JSON)으로 바뀌고, 그 기준이 화면에서 검증 가능하다는 점을 보여줌 |
| FE 흐름 검증     | 데이터셋 선택, 케이스 전환, 기대값/실제값 비교, 필드별 정확도 확인 흐름을 점검         |
| 실패 대응 확인   | API 키, 네트워크, 모델 결과 불일치 상황에서도 저장 결과 기반으로 시연을 이어갈 수 있는지 확인 |

#### 1. 시연 준비 체크리스트

| 구분        | 확인 내용                                                                 |
| ----------- | ------------------------------------------------------------------------- |
| 환경 변수   | `test/.env`에 OpenAI API 키가 설정되어 있는지 확인                         |
| 의존성      | `cd test && npm install` 완료 여부 확인                                    |
| 결과 파일   | `npm run demo` 또는 `npm run ai:test` 실행 후 `public/latest-results.json` 생성 확인 |
| FE 화면     | Vite 시연 화면에서 데이터셋 카드, 케이스 탭, 정확도 테이블이 정상 노출되는지 확인 |
| 대체 자료   | API 실패 시 보여줄 저장 결과 JSON과 콘솔 로그를 사전에 확보                 |

#### 2. 진행 순서

| 순서 | 진행 내용                 | 설명 포인트                                                                 |
| ---- | ------------------------- | --------------------------------------------------------------------------- |
| 1    | `npm run demo` 실행       | AI 테스트 실행과 FE dev server 실행을 한 번에 시작                           |
| 2    | 데이터셋 카드 확인        | Dataset A/B/C가 각각 정상, 스키마 변화, 모호성 케이스를 담당함을 설명        |
| 3    | TC-01 또는 TC-04 시연     | 명확한 질문이 ranking/comparison 기준으로 안정적으로 구조화되는 흐름을 보여줌 |
| 4    | TC-09 또는 TC-10 시연     | 컬럼명이 달라도 같은 의미의 지표로 매핑되는 스키마 유연성을 강조             |
| 5    | TC-11 또는 TC-12 시연     | 날짜 충돌이나 존재하지 않는 차원을 억지로 답하지 않고 경고/거절하는 점을 설명 |
| 6    | 결과 요약 확인            | exact match, field accuracy, hard-fail 필드를 기준으로 성공 여부를 정리      |

#### 3. FE 시연 멘트 기준

- "Logue는 답만 보여주는 서비스가 아니라, 어떤 기준으로 답했는지 먼저 보여주는 서비스입니다."
- "같은 질문이라도 컬럼명이 다른 CSV에서 의미 기준으로 다시 매핑되는지를 확인합니다."
- "모호한 질문은 임의로 계산하지 않고, 사용자가 수정할 수 있게 경고를 제공합니다."
- "실시간 API가 불안정해도 저장된 `latest-results.json`으로 동일한 FE 시연 흐름을 유지할 수 있습니다."

#### 4. self_demo Plan B/C

| 상황              | 대응 방식                                                                            |
| ----------------- | ------------------------------------------------------------------------------------ |
| API 키 미설정     | 사전 생성된 `latest-results.json`을 유지하고 FE 화면 중심으로 시연                    |
| 모델 응답 지연    | 콘솔 실행을 중단하지 않고 저장 결과 화면으로 전환                                     |
| 일부 케이스 실패  | 실패 케이스 표를 그대로 보여주고, 어떤 필드가 실패했는지 정확도 테이블에서 설명        |
| 로컬 서버 문제    | 빌드 산출물 또는 캡처 화면을 사용해 데이터셋/케이스/정확도 흐름을 순서대로 설명        |
